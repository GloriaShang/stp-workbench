...
| 日历 | 日 / 3 天 / 周 / 月视图，点日期数字只看那一天。上课、截止、待办、建议学习块四层可开关。点时间轴空白新建定时待办，点「＋」或全天栏新建当天待办；拖动移动、拖底边改时长、点方框勾选。待办按文字里提到的课程自动着色 |
...
6. 没连 Obsidian 时，新建的待办先存在工作台（同样自动保存）；连上后自动搬进对应日期的 Daily Matter。
7. 连上 vault 后，工作台数据会自动备份到 `<vault>/.stp-workbench/`；换网址、换浏览器后重新连上即可恢复。
